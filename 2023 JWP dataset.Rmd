---
Data: "2023 JWP data"
title: "Remnant Prairie Climate Climate Interactions - 2022-2023"
...
## Create function to identify and install missing packages
...
## Use function to install packages
...
# Plant Data
  ...
  1. **1979-2020 Surveys**
    ...
    - `c_value_variations.csv`: plot, mean_wt_c, year, total_cover, total_richness, change_in_c
    - `filtered-plots.csv`: plot, mean_wt_c, year
    - `jwp_multi_year_survey.csv`: genus, species, percent_cover, plot,native, richness, year, c_value
    - `jwp_species_frequency.csv`: genus, species, native, n, fraction, year, plot
    - `jwp_variables.csv`: plot, mean_wt_c, year, total_cover, total_richness
    - `native_species_freq_variation.csv`: genus, species, native, n, fraction, year, plot, change_species_freq, mean_native_species_freq
    - `nonnative_species_freq_variation.csv`: genus, species, native, n, fraction, year, plot, change_species_freq, mean_nonnative_species_freq
    - `richness_variations.csv`: plot, mean_wt_c, year, total_cover, total_richness, richness_change
  
  2. **2020 Survey**  
  
    - `jwp_survey.csv`: species, percent_cover, plot, native, richness, c_value
    - `mean_wt_c.csv`: plot, mean_wt_c
    - `species_frequency.csv`: species, n, fraction
    - `total_cover.csv`: plot, total_cover
    - `total_richness.csv`: plot, total_richness
    

## Recent Plant Community Insights

In 2020, what was the site:  

  - mean, min, and max weighted conservatism?
  - most frequently occurring species?
  - mean, min, and max total cover?
  - mean, min, and max total species richness?


```{r jwp-mean-wt-c}
read_csv("data/plants/2020-survey/mean_wt_c.csv") %>% 
  summarize(site_mean_wt_c = mean(mean_wt_c),
            plot_min_wt_c = min(mean_wt_c),
            plot_max_wt_c = max(mean_wt_c))
```

```{r jwp-freq-sp}
read_csv("data/plants/2020-survey/species_frequency.csv") %>% 
  slice_max(n, n = 5)
```

```{r jwp-total-cover}
read_csv("data/plants/2020-survey/total_cover.csv") %>% 
   summarize(site_total_cover = mean(total_cover),
            plot_min_total_cover = min(total_cover),
            plot_max_total_cover = max(total_cover))
```

```{r jwp-total-richness}
read_csv("data/plants/2020-survey/total_richness.csv") %>% 
  summarize(site_total_richness = mean(total_richness),
            plot_min_total_richness = min(total_richness),
            plot_max_total_richness = max(total_richness))
```

The most frequently occuring species were:  
...
The overall site mean weighted conservatism was 4.4 and ranged from a plot-level minimum of zero to a maximum of 8.2.
The overall site mean total cover was 9.9 and ranged from a plot-level minimum of 3 to a maximum of 21.

# Meteorology Data

## Precip and temperature conditions during 2022-2023, and for study periods

```{r get-data}
met_data <- read_csv("data/meteorology/jwp_2022_2023_weather_tidy.csv") %>% 
    mutate(date= as.Date(date),
           yday= yday(date),
           month=month(date),
           year = year(date)) %>% 
  relocate(year, month, yday, date) %>% 
  select(1:4, 6, 19)
met_data
```

What is the min, max, and mean monthly average temperature and precipiation?

```{r monthly-weather-stats}
met_data %>% 
  group_by(year, month) %>% 
  summarize(mmt = mean(temp_avg, na.rm = T), # mean monthly temperature
            cmp = sum(precip_accum, na.rm = T)) # cumulative monthly precipiation 
```

In the 2022 study period, mean monthly average temperatures ranged from 23.4 in July to 5.9 in Novermber. 
In the 2023 study period, mean monthly average temperatures ranged from 2.8 in January to 22.8 in July.

In the 2022 study period, cumulative monthly precipitation ranged from 149.9 mm in July to 22.9 in November.
In the 2023 study period, mean monthly precipitation ranged from 20.3 mm in May to 228.6 in July.


What does the study period seasonal pattern of moisture and tempertaure look like?

```{r seasonal-temp-moisture-pattern}
met_data %>% 
  group_by(year, month) %>% 
  summarize(date = median(date),
            mmt = mean(temp_avg, na.rm = T), # mean monthly temperature
            cmp = sum(precip_accum, na.rm = T)) %>% 
  mutate(study_period = case_when(
    year == 2022 & month %in% c(7:11) ~ "Sampling",
    year == 2023 & month %in% c(3, 5:8) ~ "Sampling",
    T ~ "No sampling"
  )) %>% 
  ggplot(aes(x = date, y = mmt)) + 
  geom_line(color = "grey") +
  geom_point(aes(x = date, y = mmt, color = study_period), size = 4) +
  labs(x = "Date",
       y = expression("Mean Monthly Temp. ("*degree*"C)"),
       color = "Sampling Dates") +
  theme_classic() +
  scale_color_manual(values = c("grey80", "red"))
ggsave("figures/temp-pattern.png", width = 6, height = 4)
```

```{r seasonal-precip-pattern}
met_data %>% 
  group_by(year, month) %>% 
  summarize(date = median(date),
            mmt = mean(temp_avg, na.rm = T), # mean monthly temperature
  ...
  ggplot(aes(x = date, y = cmp)) + 
  geom_point(aes(x = date, y = cmp, color = study_period), size = 4) +
  geom_segment(aes(x = date, xend = date, y = 0, yend = cmp, color = study_period)) +
  labs(x = "Date",
       y = expression("Cum. Monthly Precip. (mm)"),
       color = "Sampling Dates") +
  theme_classic() +
  scale_color_manual(values = c("grey80", "blue"))
ggsave("figures/precip-pattern.png", width = 6, height = 4)
```
...
Get the soil data: `soil_moist_temp23.csv`

```{r get-soil-data}
read_csv("data/soil/soil_moist_temp23.csv",
         na = "na") %>%
  mutate(date = str_replace(date, "/", "-"),
         date = str_replace(date, "/", "-"),
         date = mdy(date),
         
         plant_cover_percent = case_when(
           plant_cover_percent == "<5" ~ "5",
           T ~ plant_cover_percent),
         
         plant_cover_percent = as.numeric(plant_cover_percent)) %>% 
    group_by(date) %>% 
     summarize(mean_soil_temp = mean(soil_temp_c, na.rm = T),
            mean_soil_moist = mean(soil_moist_percent, na.rm = T),
            mean_plant_cover = mean(plant_cover_percent, na.rm = T)) %>% 
  arrange(date)
```

It is missing 2022, and the data seem very strange! Trying another dataset.

Get the soil data: `export-soil-temp-data.csv`

```{r get-soil-data-2}
soil_temp_moist <- read_csv("data/soil/export-soil-temp-data.csv",
         na = "NA",
         col_names = 
           c("row",
             "plot",
...
Plot soil temperature by date and plot
  
```{r soil-temp-dynamics}
soil_temp_moist %>% 
  # rename(microtopography = plot) %>% 
  pivot_longer(3:4, names_to = "soil_var", values_to = "values") %>% 
  group_by(plot, date) %>%
  mutate(year = year(date),
         soil_var = fct_relevel(soil_var, "mean_soil_temp", "mean_soil_moist"),
         values = case_when(
           date == "2022-10-19" & soil_var == "mean_soil_moist" ~ NA,
           T ~ values
         )) %>% 
  relocate(year, date) %>% 
  ggplot(aes(x = date, 
  ...
  facet_wrap(~soil_var, 
             ...
             labeller = labeller(
               soil_var = c(
                "mean_soil_temp" = "TSoil (°C)",
                "mean_soil_moist"=   "VWC (%)") 
             )) +
  # scale_color_manual(values = c("#1c2ed6", "#ce0ef0")) +
  scale_alpha_manual(values = c(0.25, 0.5, 0.75, 1)) +
  labs(title = "Seasonal Pattern in Soil Temperature", 
     subtitle = "On gas flux sampling days",
     x = "Day", 
     y = NA,
     shape = "Plot",
     alpha = "Plot") +
...
Temp: Very little difference among plots, but major seaosonal pattern.

Big differences among plots, less of pronounced seasonal pattern.

**I'm going to remove the anomalous values in Fall 2022. They look to be biased low.

Final plot:

```{r final-soil-dynamics}
soil_temp_moist %>% 
  # rename(microtopography = plot) %>% 
  pivot_longer(3:4, names_to = "soil_var", values_to = "values") %>% 
  group_by(plot, date) %>%
  ...
  geom_point(size = 3, alpha = c(0.75)) +
  facet_wrap(~soil_var, 
             ncol = 1,
             scales = "free_y",
             labeller = labeller(
