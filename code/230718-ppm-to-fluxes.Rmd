...
```{r}
ghg_ppm <-read_csv("../data/tidy/230718/230718_ghg_ppm.csv") %>% 
...
```{r}
min_elapsed <- read_csv("../data/tidy/230718/min_elapsed.csv") %>%
...
ggsave("data/diagnostics/230718_co2_lm.png")
```
...
ggsave("data/diagnostics/230718_ch4_lm.png")
```
...
ggsave("data/diagnostics/230718_n2o_lm.png")
```
...
ghg_min_elapsed %>%
  ...
  write_csv("../data/final/230718/co2_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230718/ch4_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230718/n2o_fluxes.csv")
```

## get fluxes to plot

```{r read}
co2 <- read_csv("../data/final/230718/co2_fluxes.csv") %>% 
  mutate(gas = "co2")
```

```{r}
ch4 <- read_csv("../data/final/230718/ch4_fluxes.csv") %>% 
  mutate(gas = "ch4")
```

```{r}
n2o <- read_csv("../data/final/230718/n2o_fluxes.csv") %>% 
  mutate(gas = "n2o")
```
...
```{r chamber-temps}
chamber_temps <- read_csv("../data/tidy/230718/chamber_temps.csv")
...
```{r}
write_csv(fluxes, "../data/final/230718/fluxes.csv")
```
