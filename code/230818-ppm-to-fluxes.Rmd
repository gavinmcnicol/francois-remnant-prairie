...
```{r}
ghg_ppm <-read_csv("../data/tidy/230818/230818_ghg_ppm.csv") %>% 
...
```{r}
min_elapsed <- read_csv("../data/tidy/230818/min_elapsed.csv") %>%
...
```{r combine-ghg-ppm-with-min-elapsed}
ghg_minelapsed <- ghg_ppm %>% 
  left_join(min_elapsed, by = c("chamber", "timepoint"))
```
...
ggsave("data/diagnostics/230818_co2_lm.png")
```
...
ggsave("data/diagnostics/230818_ch4_lm.png")
```
...
ggsave("data/diagnostics/230818_n2o_lm.png")
```
...
ghg_min_elapsed %>%
  ...
  write_csv("../data/final/230818/co2_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230818/ch4_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230818/n2o_fluxes.csv")
```

## get fluxes to plot

```{r read}
co2 <- read_csv("../data/final/230818/co2_fluxes.csv") %>% 
  mutate(gas = "co2")
```

```{r}
ch4 <- read_csv("../data/final/230818/ch4_fluxes.csv") %>% 
  mutate(gas = "ch4")
```

```{r}
n2o <- read_csv("../data/final/230818/n2o_fluxes.csv") %>% 
  mutate(gas = "n2o")
```
...
```{r chamber-temps}
chamber_temps <- read_csv("../data/tidy/230818/chamber_temps.csv")
...
```{r}
write_csv(fluxes, "../data/final/230818/fluxes.csv")
```
