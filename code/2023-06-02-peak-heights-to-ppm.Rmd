...
list the .txt files to read from the second date `6.2.23`

```{r list-files}
files <- list.files("../data/gc-raw/JWP 2023/6.2.23", pattern = ".txt")
length(files)
```
...
```{r read-sample-log}
sample_log <- read_csv("../data/gc-raw/JWP 2023/6.2.23/sample_log.csv", na = c("N/a", "<NA>")) %>% 
  janitor::clean_names() %>% 
  mutate(date_collected = mdy(substr(filename, 1, 8)),
         gc_run_date = mdy("060523")) %>% 
...
data <- lapply(paste0("../data/gc-raw/JWP 2023/6.2.23/", files), read_fwf)
data[[96]]
...
co2_height <- bind_cols(co2_height_1, co2_height_2, co2_height_3) %>% 
  ...
  mutate(col1 = ifelse(col1 < 100000, NA, col1),
         col2 = ifelse(col2 < 950000 | col2 > 150000, NA, col2),
...
```{r}
write_csv(gc_data_tidy, "../data/tidy/230602/230602_gc_peak_heights.csv")
```

## Re-read in tidy data

```{r}
gc_data_tidy <- read_csv("../data/tidy/230602/230602_gc_peak_heights.csv") %>% 
 glimpse()
```
...
```{r}
write_csv(ghg_ppm, "../data/tidy/230602/230602_ghg_ppm.csv")
```
