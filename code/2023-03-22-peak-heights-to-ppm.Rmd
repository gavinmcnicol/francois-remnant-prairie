...
```{r list-files}
files <- list.files("../data/gc-raw/JWP 2023/3.22.23", pattern = ".txt")
length(files)
```
...
```{r read-sample-log}
sample_log <- read_csv("../data/gc-raw/JWP 2023/3.22.23/sample_log.csv", na = c("N/a", "<NA>")) %>% 
  janitor::clean_names() %>% 
  mutate(date_collected = mdy(substr(filename, 1, 6)),
         gc_run_date = mdy("052323")) %>% 
  mutate(row = as.numeric(substr(filename,8,10)))
...
data <- lapply(paste0("../data/gc-raw/JWP 2023/3.22.23/", files), read_fwf)
data[[6]]
```
...
ch4_height <- bind_cols(ch4_height_1, ch4_height_2, ch4_height_3) %>% 
  rename("col1" = `...1`,
         "col2" = `...2`,
         "col3" = `...3`) %>% 
  mutate(col1 = ifelse(col1 < 1000 | col1 > 10000, NA, col1),
         col2 = ifelse(col2 < 9500 | col2 > 11500, NA, col2),
         col3 = ifelse(col3 < 1000 | col3 > 10000, NA, col3)) %>% 
...
n2o_height <- bind_cols(n2o_height_2, n2o_height_3, n2o_height_4, n2o_height_5) %>% 
  rename("col1" = `...1`,
         ...
         "col3" = `...3`,
         "col4" = `...4`) %>% 
  mutate(col1 = ifelse(col1 < 400, NA, col1),
         col2 = ifelse(col2 < 400, NA, col2),
         col3 = ifelse(col3 < 400, NA, col3),
         col4 = ifelse(col4 < 400, NA, col4)) %>% 
...
```{r}
write_csv(gc_data_tidy, "../data/tidy/230322/230322_gc_peak_heights.csv")
```

## Re-read in tidy data

```{r}
gc_data_tidy <- read_csv("../data/tidy/230322/230322_gc_peak_heights.csv") %>% 
...
```{r}
write_csv(ghg_ppm, "../data/tidy/230322/230322_ghg_ppm.csv")
```
