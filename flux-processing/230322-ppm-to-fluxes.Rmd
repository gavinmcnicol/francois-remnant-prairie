...
```{r}
ghg_ppm <-read_csv("../data/tidy/230322/230322_ghg_ppm.csv")
...
```{r}
min_elapsed <- read_csv("../data/tidy/230322/min_elapsed.csv") %>%
...
ghg_min_elapsed %>% 
  ...
  # strip.text.x = element_blank(), to set the title blanck
)
ggsave("data/diagnostics/230322_co2_lm.png")
```
...
ggsave("data/diagnostics/230322_ch4_lm.png")
```
...
ggsave("data/diagnostics/230322_n2o_lm.png")
```
...
ghg_min_elapsed %>%
  ...
  write_csv("../data/final/230322/co2_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230322/ch4_fluxes.csv")
```
...
ghg_min_elapsed %>%
  ...
  filter(term == "min_elapsed") %>% 
  write_csv("../data/final/230322/n2o_fluxes.csv")
```

## get co2 fluxes to plot

```{r read}
co2 <- read_csv("../data/final/230322/co2_fluxes.csv") %>% 
  mutate(gas = "co2")
```
...
## get ch4 fluxes to plot

```{r}
ch4 <- read_csv("../data/final/230322/ch4_fluxes.csv") %>% 
  mutate(gas = "ch4")
``` 

## get n2o fluxes to plot
  
```{r}
n2o <- read_csv("../data/final/230322/n2o_fluxes.csv") %>% 
  mutate(gas = "n2o")
```
...
```{r chamber-temps}
chamber_temps <- read_csv("../data/tidy/230322/chamber_temps.csv")%>%  mutate(date = ymd(date))
...
```{r merge}
 fluxes <- co2 %>% bind_rows(ch4) %>% bind_rows(n2o) %>% 
  ...
  select(-temp_start, -temp_end, -ecosystem_name, -ecosystem_block, -rep) %>% 
...
```{r}
write_csv(fluxes, "../data/final/230322/fluxes.csv")
```
