...
```{r list-files}
files <- list.files("../data/gc-raw/JWP 2023/7.6.23", pattern = ".txt")
length(files)
```
...
```{r read-sample-log}
sample_log <- read_csv("../data/gc-raw/JWP 2023/7.6.23/sample_log.csv", na = c("N/a", "<NA>")) %>% 
  janitor::clean_names() %>% 
  mutate(date_collected = mdy(substr(filename, 1, 8)),
         gc_run_date = mdy("082423")) %>% 
...
data <- lapply(paste0("../data/gc-raw/JWP 2023/7.6.23/", files), read_fwf)
data[[6]]
```
...
```{r}
write_csv(gc_data_tidy, "../data/tidy/230706/230706_gc_peak_heights.csv")
```

## Re-read in tidy data

```{r}
gc_data_tidy <- read_csv("../data/tidy/230706/230706_gc_peak_heights.csv")
```
```{r}
gc_data_tidy
```
...
```{r}
write_csv(ghg_ppm, "../data/tidy/230706/230706_ghg_ppm.csv")
```
