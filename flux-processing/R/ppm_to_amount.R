#------------------------------------------------------------
# Step 3: ppm -> amount using chamber vols + temps
#------------------------------------------------------------
ppm_to_amount <- function(ppm_df,
                          chamber_vols,
                          chamber_temps_file,
                          pressure_atm = 1) {
  ...
  out <- ppm_df %>%
    ...
    mutate(
      ...
      ch4_ppm = as.numeric(ch4_ppm),
      n2o_ppm = as.numeric(n2o_ppm),
      
      # moles of air in chamber
      n_air_mol = (pressure_atm * total_vol_m3) / (R * average_temp_kelvin),
      
      # ppm -> mol fraction -> mol gas
  ...
  # hard fail on missing joins (prevents silent NA propagation)
  bad <- out %>%
    summarize(
  ...
  if (bad$n_missing_vol > 0 || bad$n_missing_tmp > 0 || bad$n_missing_rep > 0) {
    stop("ppm_to_amount(): join failures: missing volume=", bad$n_missing_vol,
         " missing temp=", bad$n_missing_tmp,
